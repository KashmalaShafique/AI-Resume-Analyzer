...
const AIMatching = () => {
  ...
  const parseResume = async (resumeId) => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:8000/api/matching/parse/${resumeId}`,
        {},
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
      ...
      if (response.data.message) {
        ...
        fetchJobs(); // Refresh to show updated data
      }
    } catch (error) {
  ...
  const matchResumesWithJob = async (jobId) => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:8000/api/matching/match/${jobId}`,
        {},
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
      ...
      alert('Error matching resumes. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const getMatchResults = async (jobId) => {
    try {
      const response = await axios.get(
        `http://localhost:8000/api/matching/results/${jobId}`,
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
  ...
  const exportResults = async (jobId) => {
    try {
      const response = await axios.get(
        `http://localhost:8000/api/export/matches/csv/${jobId}`,
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          },
      ...
      link.href = url;
      link.setAttribute('download', `job_${jobId}_matches.csv`);
    ...
    <div className="min-h-screen py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-12 text-center">
          <h1 className="text-5xl font-bold text-white mb-4">AI Resume Matching</h1>
          <p className="text-xl text-gray-300 max-w-3xl mx-auto">
            Use advanced AI to match resumes with job descriptions and find the best candidates automatically
          </p>
        </div>
        ...
        <div className="bg-gray-800 rounded-2xl shadow-2xl mb-8 overflow-hidden border border-gray-700">
          <div className="bg-gradient-to-r from-blue-600 to-purple-600 px-8 py-6">
            <h2 className="text-2xl font-bold text-white">Select Job Description</h2>
            <p className="text-blue-100 mt-2">Choose a job to match with your resumes</p>
          </div>
          <div className="p-8">
            {jobs.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {jobs.map((job) => (
                  <div key={job.id} className="bg-gradient-to-br from-gray-700 to-gray-800 border-2 border-gray-600 rounded-2xl p-6 hover:shadow-2xl hover:border-blue-400 transition-all duration-300">
                    ...
                    <div className="flex flex-col space-y-3">
                      <button
                        onClick={() => matchResumesWithJob(job.id)}
                        disabled={loading}
                      ...
                      <div className="flex space-x-2">
                        <button
                          onClick={() => getMatchResults(job.id)}
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-300 mb-6">Create your first job description to start matching resumes</p>
        ...
        {matchResults.length > 0 && (
          <div className="bg-gray-800 rounded-2xl shadow-2xl overflow-hidden border border-gray-700">
            <div className="bg-gradient-to-r from-green-600 to-emerald-600 px-8 py-6">
              ...
              <p className="text-green-100 mt-2">AI-powered candidate analysis and ranking</p>
            </div>
            <div className="p-8">
              <div className="space-y-6">
                {matchResults.map((result, index) => (
                  <div key={index} className="bg-gradient-to-br from-gray-700 to-gray-800 border-2 border-gray-600 rounded-2xl p-6 hover:shadow-2xl transition-all duration-300">
                    <div className="flex justify-between items-start mb-6">
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          <h3 className="text-xl font-bold text-white">{result.resume_name}</h3>
                          <p className="text-gray-300">Candidate #{index + 1}</p>
                        </div>
                      </div>
                      ...
                      <div className="bg-gray-600 rounded-xl p-4 border border-gray-500">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {result.matched_skills && result.matched_skills.map((skill, idx) => (
                            <span key={idx} className="bg-green-600 text-green-100 text-sm px-3 py-1 rounded-full font-medium">
                              {skill}
                            </span>
                          ))}
                        </div>
                      ...
                      <div className="bg-gray-600 rounded-xl p-4 border border-gray-500">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {result.missing_skills && result.missing_skills.map((skill, idx) => (
                            <span key={idx} className="bg-red-600 text-red-100 text-sm px-3 py-1 rounded-full font-medium">
                              {skill}
                            </span>
                          ))}
                        </div>
